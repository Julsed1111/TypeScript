...
type Badge = `${BadgeSizeType}_${PrintType}`;

class Student {
	badgeTypeMap = new Map<Badge, BadgeTypesEnum>([
	...
	getPerformanceRating(): number {
		...
		const averageGrade = gradeValues.reduce((sum: number, grade: Grade) => sum + (grade.mark ? 1 : 0), 0) / gradeValues.length;
